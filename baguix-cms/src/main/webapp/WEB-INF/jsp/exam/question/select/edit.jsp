...
<form id="exam_select_add_form" method="post">
            ...
            <div title="题目正文" data-options="iconCls:'icon-article'" style="overflow:auto;padding:6px;">
                ...
                <script type="text/javascript">
                    ...
                    contentEditor.ready(function() {
                        contentEditor.setContent('${select.content}');
                    });
                </script>
            </div>
            <div title="题目解析" data-options="iconCls:'icon-help'" style="overflow:auto;padding:7px;">
                <script id="analysis" name="analysis" value="${select.analysis}" type="text/plain"
                        style="width:772px;height:100px;"></script>
                <script type="text/javascript">
                    var analysisEditor = UE.getEditor('analysis');
                    analysisEditor.ready(function() {
                        analysisEditor.setContent('${select.analysis}');
                    });
                </script>
            </div>
            <div title="标签" data-options="iconCls:'icon-dictionary'" style="text-align:center;overflow:auto;padding:7px;">
                <ss:SysDict dictname="ExamTags" name="etags" id="etags" init="'${select.tags}'"></ss:SysDict>
            </div>
        </div>
    </div>
    <div data-options="region:'south',border:false" style="height:225px;">
                        ...
                        $.messager.confirm('操作确认', '该单选题已经存在答案项，您是否确认将此项设为答案项？', function (r) {
                            if (r) {
                                $('#options').datagrid('appendRow', {
                                    answer: $('#isanswer').switchbutton("options").checked?"true":"false",
        ...
        <table id="options" title="选项列表" class="easyui-datagrid"
               data-options="url:'${pageContext.request.contextPath}/exam/question/select/options.do?id=${select.id}',rownumbers:true,border:false,fit:true,fitColumns:true,singleSelect:true,toolbar:'#toolbar'"
...
<script type="text/javascript">
    ...
    function postaddForm(){
        ...
        q.kid = '${select.kid}';
        q.tags=$('#etags').val();
        $.post('${pageContext.request.contextPath}/exam/question/select/edit_deal.do', q,
                function (data) {
                    if (data.success) {
                         // 在grid中插入新增行
